...
from database_requests import (
    list_faculty, get_faculty_profile, update_faculty_profile,
    post_comment, list_comments, edit_comment, delete_comment,
...
def login_view(request):
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT fid, email, password, name, auth_level FROM Faculty WHERE email = %s", [email])
            row = cursor.fetchone()
...
def faculty_delete_view(request, fid):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("DELETE FROM Preferred_Start_Times WHERE config_id = %s", [fid])
        ...
        return JsonResponse({"success": True, "message": "Faculty and associated data deleted"})
...
def list_configurations(request):
    ...
    with connection.cursor() as cursor:
        ...
        for config_id, travel_time in configs:
            cursor.execute("SELECT days FROM Preferred_Days WHERE config_id = %s", [config_id])
            days = [row[0] for row in cursor.fetchall()]

            cursor.execute("SELECT times FROM Preferred_Start_Times WHERE config_id = %s", [config_id])
...
def create_configuration(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("INSERT INTO Configuration (travel_time) VALUES (%s)", [travel_time])
        cursor.execute("SELECT LAST_INSERT_ID()")
        config_id = cursor.fetchone()[0]

        for day in days:
            cursor.execute("INSERT INTO Preferred_Days (config_id, days) VALUES (%s, %s)", [config_id, day])

        for t in times:
            cursor.execute("INSERT INTO Preferred_Start_Times (config_id, times) VALUES (%s, %s)", [config_id, t])
...
def update_configuration(request, config_id):
    ...
    with connection.cursor() as cursor:
        cursor.execute("UPDATE Configuration SET travel_time = %s WHERE config_id = %s", [travel_time, config_id])

        cursor.execute("DELETE FROM Preferred_Days WHERE config_id = %s", [config_id])
        for day in days:
            cursor.execute("INSERT INTO Preferred_Days (config_id, days) VALUES (%s, %s)", [config_id, day])

        cursor.execute("DELETE FROM Preferred_Start_Times WHERE config_id = %s", [config_id])
        for t in times:
            cursor.execute("INSERT INTO Preferred_Start_Times (config_id, times) VALUES (%s, %s)", [config_id, t])
